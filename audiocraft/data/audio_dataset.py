...
class OnMemoryAudioDataset:
    ...
    def load_audios(self):
        ...
        for i in tqdm(range(len(self.meta))):
            file_meta = self.meta[i]
            out, sr = torchaudio.load(file_meta.path)
            # out = convert_audio(out, sr, self.sample_rate, self.channels) # if dataset is not 32000khz, convert to 32000khz
            audios.append(out.to(torch.float16))
        return audios
    ...
    def sample_file(self, index: int, rng: torch.Generator) -> AudioMeta:
        ...
        if self.permutation_on_files:
            ...
            permutation = AudioDataset._get_file_permutation(
                len(self.meta), permutation_index, self.shuffle_seed)
            file_index = permutation[relative_index]
            return self.meta[file_index], file_index
    ...
    def __getitem__(self, index: int, quantize_seek_time = False) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, SegmentInfo]]:
        if self.segment_duration is None:
            ...
            for retry in range(self.max_read_retry):
                file_meta, file_index = self.sample_file(index, rng)
                # We add some variance in the file position even if audio file is smaller than segment
                # without ending up with empty segments
                max_seek = max(0, file_meta.duration - self.segment_duration * self.min_segment_ratio)
                if quantize_seek_time: # For MIDIAudioDataset
                ...
                try:
                    # out, sr = audio_read(file_meta.path, seek_time, self.segment_duration, pad=False)
                    # out = convert_audio(out, sr, self.sample_rate, self.channels)
                    out = self.audios[file_index]
                    out = out[..., int(seek_time * self.sample_rate):int((seek_time + self.segment_duration) * self.sample_rate)]
                    n_frames = out.shape[-1]
                    target_frames = int(self.segment_duration * self.sample_rate)
                    if self.pad:
                        out = F.pad(out, (0, target_frames - n_frames))
        ...
        if self.return_info:
            # Returns the wav and additional information on the wave segment
            return out.to(torch.float32), segment_info
        else:
