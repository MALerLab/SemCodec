...
def slice_piano_roll_dict(piano_roll):
    piano_roll_segments = {}
    piano_roll_segments['len']=int(piano_roll.shape[-1]//3000)+1
    for i in range(piano_roll_segments['len']-1):
        piano_roll_segments[i*30]=(torch.Tensor(piano_roll[...,i*3000:(i+1)*3000]).to(torch.int8))
    piano_roll_segments[-1]=(torch.Tensor(piano_roll[...,-3000:]).to(torch.int8))
...
for filename in tqdm(val_split):
    print("Piano rolling: ", filename)
    parsed_midi = parse_midi(str(filename))
    piano_roll = create_piano_roll(parsed_midi)
    piano_rolls_dict = slice_piano_roll_dict(piano_roll)
    
    torch.save(piano_rolls_dict, str(filename).rsplit('.', 1)[0]+'_bt_pianoroll.pkl')
...
for dirpath, dirnames, filenames in os.walk("/home/jongmin/userdata/MAESTRO/maestro-v3.0.0/"):
    for filename in tqdm([f for f in filenames if f.endswith(".midi")]):
        ...
        torch.save(piano_rolls_dict, str(os.path.join(dirpath, filename)).rsplit('.', 1)[0]+'_bt_pianoroll.pkl')