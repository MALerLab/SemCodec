...
class SemCodecMidiDecoderTrainer:
    ...
    def train(self, train_loader, epoch, onset_threshold, frame_threshold):
        ...
        for batch_idx, (encodec_tensor, pr_tensor) in tqdm(enumerate(train_loader)):
            ...
            if prediction.sum().item() == 0 or pr_tensor.sum().item() == 0:
                ...
                wandb.log({"frame_f1": frame_f1, "frame_precision": frame_precision, "frame_recall": frame_recall, 'iter':self.iteration})
            loss = self.criterion(output, pr_tensor)
            loss.backward()
            self.optimizer.step()

            #mir_eval
            if self.iteration % 2000 == 0 and self.iteration != 0:
            ...
            wandb.log({"loss": loss.item(), "epoch": epoch, "batch_idx": batch_idx, "accuracy": acc, 'iter':self.iteration})
            self.iteration += 1
        # self.scheduler.step()
        ...
        with torch.no_grad():
            for batch_idx, (encodec_tensor, pr_tensor) in tqdm(enumerate(val_loader)):
                ...
                if prediction.sum().item() == 0 or pr_tensor.sum().item() == 0:
                    ...
                    wandb.log({"val_frame_f1": val_frame_f1, "val_frame_precision": val_frame_precision, "val_frame_recall": val_frame_recall, 'iter':self.iteration})
    ...
    def val_inference(self, validset, idx, save_target, epoch):
        ...
        with torch.no_grad():
            ...
            wandb.log({f"val_inference/{idx}/wav": wandb.Audio(pred_wav.T[0], caption=f"pred_wav_{epoch}", sample_rate=44100), 'iter':self.iteration})
            plt.figure(figsize = (25,10))
            pred_onset_plt = plt.imshow(prediction[0,0].cpu().numpy(), aspect='auto', origin='lower', interpolation='none')
            wandb.log({f"val_inference/{idx}/onset": wandb.Image(pred_onset_plt, caption=f"pred_onset_{epoch}"), 'iter':self.iteration})
            plt.close('all')
            plt.figure(figsize = (25,10))
            pred_sustain_plt = plt.imshow(prediction[0,1].cpu().numpy(), aspect='auto', origin='lower', interpolation='none')
            wandb.log({f"val_inference/{idx}/sustain": wandb.Image(pred_sustain_plt, caption=f"pred_sustain_{epoch}"), 'iter':self.iteration})    
            plt.close('all')    
            ...
            if save_target:
                ...
                wandb.log({f"val_inference/{idx}/target/wav": wandb.Audio(target_wav.T[0], caption=f"target_wav_{epoch}", sample_rate=44100)})
                plt.figure(figsize = (25,10))
                target_onset_plt = plt.imshow(pr_tensor[0].cpu().numpy(), aspect='auto', origin='lower', interpolation='none')
                wandb.log({f"val_inference/{idx}/target/onset": wandb.Image(target_onset_plt, caption=f"target_onset_{epoch}")})
                plt.close('all')
                plt.figure(figsize = (25,10))
                target_sustain_plt = plt.imshow(pr_tensor[1].cpu().numpy(), aspect='auto', origin='lower', interpolation='none')
                wandb.log({f"val_inference/{idx}/target/sustain": wandb.Image(target_sustain_plt, caption=f"target_sustain_{epoch}")})
                plt.close('all')
...
def main():
    ...
    model.load_state_dict(torch.load("semcodec_decoder_distinctive-snowball-5_1400.pt")[0])
    optimizer.load_state_dict(torch.load("semcodec_decoder_distinctive-snowball-5_1400.pt")[1])
    ...
    print(run.name)
    for i in tqdm(range(1401,epochs)):
        ...
        if i % 100 == 0 and i != 0:
            ...
            torch.save([model.state_dict(), optimizer.state_dict()], f'semcodec_decoder_{run.name}_{i}.pt')
